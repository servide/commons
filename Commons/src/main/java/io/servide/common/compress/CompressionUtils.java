...
public enum CompressionUtils {
  ...
  public static byte[] decompress(byte[] data) {
    ...
    while (!inflater.finished()) {
      int count = Try.to(() -> inflater.inflate(buffer));
      outputStream.write(buffer, 0, count);
    }
  ...
  public static byte[] compressSmallString(String inString) {
  ...
  private static void outputVerb(ByteArrayOutputStream baos, String str) {
    if (str.length() == 1) {
      ...
      Try.to(() -> baos.write(str.getBytes()));
    }
  }
  ...
  public static String decompressSmallString(byte[] strBytes) {
